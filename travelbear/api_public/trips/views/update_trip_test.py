...
@pytest.fixture
def api_client(user):
    return Client(HTTP_TEST_USER_EXTERNAL_ID=user.external_id)


@pytest.fixture
def call_endpoint(api_client):
    def _call_endpoint(trip_id, data):
        url = reverse(trip_id_endpoint, kwargs={"trip_id": trip_id})
        return api_client.patch(url, data=json.dumps(data))
...
def test_update_trip(call_endpoint, trip):
    ...
    response = call_endpoint(trip.trip_id, data=None)
    assert 400 == response.status_code

    response = call_endpoint(trip.trip_id, data={"title": "new title", "tags": ["camping", "mountains"]})
    ...
    response = call_endpoint(trip.trip_id, data={"is_deleted": True, "title": "foo"})
    assert 400 == response.status_code
    response_body = json.loads(response.content)
    assert Trip.objects.get(pk=trip.pk).is_deleted is False
    assert Trip.objects.get(pk=trip.pk).title != "foo"
    assert response_body["validation_errors"] == ["Cannot update one or more requested fields"]
